...
fn blocking_ssh(
    ...
    const REMOTE_SETUP_SCRIPT: &[u8] = include_bytes!("../../../scripts/ssh-setup.sh");
    ...
    ssh_send_file(
        &session,
        &mut REMOTE_SETUP_SCRIPT,
        u64::try_from(REMOTE_SETUP_SCRIPT.len()).expect("script too long"),
        &dest,
    )
    .context("failed to send test runner to remote")?;
    ...
    let args = format!(
        "{remote_dir} \"{}\" \"{}\" \"{}\"",
        local_app_manifest.current_app_path.file_name().unwrap().to_string_lossy(),
        local_app_manifest.previous_app_path.file_name().unwrap().to_string_lossy(),
        local_app_manifest.ui_e2e_tests_path.file_name().unwrap().to_string_lossy(),
    );
